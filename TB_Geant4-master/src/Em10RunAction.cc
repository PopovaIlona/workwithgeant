   ...
   nbinvertexz(0)
{
	...
	analysisManager->SetFirstHistoId(1);
		...
		// Creating histograms
	analysisManager->CreateH1("h1", "Gamma spectrum after absorber (keV)",
																	500, 0., 50.);
	analysisManager->SetH1XAxisTitle(1, "keV");
	analysisManager->SetH1YAxisTitle(1, "Entries");
	analysisManager->CreateH1("h2", "Angular distribution after absorber",	
																	300,0., 0.6);
	analysisManager->SetH1XAxisTitle(2, "rad");
	analysisManager->SetH1YAxisTitle(2, "Entries");
	analysisManager->CreateH1("h3", "Inc. gamma spectrum (keV)", 500, 0., 50.);
	analysisManager->SetH1XAxisTitle(3, "keV");
	analysisManager->SetH1YAxisTitle(3, "Entries");
	analysisManager->CreateH1("h4", "Angular distribution of inc. spectrum", 
																	300,0., 0.01);
	analysisManager->SetH1XAxisTitle(4, "rad");
	analysisManager->SetH1YAxisTitle(4, "Entries");
	analysisManager->CreateH1("h5", "Energy deposition OLD WAY", 500, 0., 150.);
	analysisManager->SetH1XAxisTitle(5, "keV");
	analysisManager->SetH1YAxisTitle(5, "Entries");
	analysisManager->CreateH2("h11", "Hit position in the detector (x vs y)",
													1001, -10, 10, 1001, -10, 10);
	analysisManager->SetH2XAxisTitle(1, "x (cm)");
	analysisManager->SetH2YAxisTitle(1, "y (cm)");
	analysisManager->CreateH3("h12", "Hit position in the detector (x vs y vs Z)",
									256, -10, 10, 256, -10, 10, 100, 2250, 2350);
	analysisManager->SetH3XAxisTitle(1, "x (cm)");
	analysisManager->SetH3YAxisTitle(1, "y (cm)");
	analysisManager->SetH3ZAxisTitle(1, "mm");
	analysisManager->CreateH2("h13", "En_vs_ang", 500,0., 0.0075, 500, 1., 100.);
	analysisManager->SetH2XAxisTitle(2, "rad");
	analysisManager->SetH2YAxisTitle(2, "keV");
	analysisManager->CreateH2("h14", "En_vs_ang_det", 500,0., 0.0075, 500, 1., 100.);
	analysisManager->SetH2XAxisTitle(3, "rad");
	analysisManager->SetH2YAxisTitle(3, "keV");
/*
	analysisManager->CreateNtuple("1", "ETP");
	analysisManager->CreateNtupleDColumn("KinE_keV");
	analysisManager->CreateNtupleDColumn("Ang");
	analysisManager->FinishNtuple();*/

	analysisManager->CreateNtuple("t1", "ETP");
	analysisManager->CreateNtupleDColumn("Energy_deposit_keV");
	analysisManager->CreateNtupleDColumn("Global_Time_ps");
	analysisManager->CreateNtupleDColumn("X_cm");
	analysisManager->CreateNtupleDColumn("Y_cm");
	analysisManager->CreateNtupleDColumn("Z_cm");
	analysisManager->CreateNtupleDColumn("KinE_keV");
	analysisManager->CreateNtupleDColumn("Ang_distr_rad");
	//analysisManager->CreateNtupleDColumn("Ang_distr_cos");
	analysisManager->FinishNtuple();


	
	analysisManager->CreateH1("h6", "anglular distribution in detector",
																	10000, 0., 0.006);
	analysisManager->SetH1XAxisTitle(6, "rad");
	analysisManager->SetH1YAxisTitle(6, "arb. units");
	analysisManager->CreateH1("h7", "Energy deposition", 1000, 0., 250.);
	analysisManager->SetH1XAxisTitle(7, "keV");
	analysisManager->SetH1YAxisTitle(7, "arb. units");
	analysisManager->CreateH1("h8", "Energy deposition2", 50, 0., 250.);
	analysisManager->SetH1XAxisTitle(8, "keV");
	analysisManager->SetH1YAxisTitle(8, "arb. units");

	analysisManager->CreateH1("h9", "Gamma spectrum after radiator (keV)", 
																		500, 0., 50.);
	analysisManager->SetH1XAxisTitle(9, "keV");
	analysisManager->SetH1YAxisTitle(9, "Entries");
	analysisManager->CreateH1("h10", "Angular distribution after radiator",
																		300,0., 0.6);
	analysisManager->SetH1XAxisTitle(10, "rad");
	analysisManager->SetH1YAxisTitle(10, "Entries");
...
void Em10RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  { 
	...
	// Save and normalize histograms
 
 G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
   G4int ihh = 1;
    G4int NbofEvents = aRun->GetNumberOfEvent();
	G4double facc = (1./(NbofEvents));
	 
	  //analysisManager->ScaleH2(ihh,facc);


  for (G4int ih = 1; ih<=10; ih++)
  {  
    G4int NbofEvents = aRun->GetNumberOfEvent();
